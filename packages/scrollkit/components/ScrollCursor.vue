...
/**
 * 커스텀 커서(점+링 이중 추적). 원본: apps/pioneer/components/app/AppCursor.vue
 * 점은 즉시, 링은 관성 있는 quickTo 트윈으로 따라오고,
 * 인터랙티브 요소 위에서 링이 확대된다. 터치 기기에서는 렌더하지 않는다.
 */
...
.cursor-dot {
  width: 6px;
  height: 6px;
  background: var(--ink, #f2f0ec);
}

.cursor-ring {
  width: 36px;
  height: 36px;
  border: 1px solid var(--ink-faint, rgba(242, 240, 236, 0.32));
}
</style>
