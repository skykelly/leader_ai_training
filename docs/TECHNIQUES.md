...
- **곡률 음영 — 이목구비를 살리는 핵심 항**: `dot(n,L)`만으로는 광원 반대쪽 절반의
  이목구비가 통째로 그림자에 잠긴다. **법선의 발산**(`d(n.x)/dx + d(n.y)/dy`)은
  조명과 무관한 순수 형태 신호다 — 볼록(콧대·눈두덩)은 양수, 오목(눈구멍·입술선)은
  음수. `vLight *= 1.0 + curv * k`로 곱하면 이목구비가 즉시 드러난다. 주의 넷:
  ① **반경 두 개를 섞을 것**(좁은 쪽=입술선·쌍꺼풀, 넓은 쪽=눈구멍·볼우물).
  한쪽만 쓰면 다른 스케일이 통째로 묻힌다.
  ② **오목/볼록을 비대칭으로** — 볼록에 같은 세기를 주면 눈두덩·입술 가장자리가
  밝은 테두리로 타올라 눈이 고글처럼, 얼굴이 가면처럼 보인다(concave 1.25 / convex 0.7).
  ③ **실루엣 가드 필수** — 미분 탭 하나가 배경으로 떨어지면 값이 폭발해 턱선·관자놀이가
  밝은 날개로 타오른다. 탭 위치의 커버리지를 검사해 하나라도 밖이면 0을 반환할 것.
  ④ 미분 간격은 맵 해상도에 묶이므로 `uTexel`을 uniform으로 받을 것(하드코딩 금지).
- **점 그림의 대비는 밀도로도 온다**: 밝기만으로 이목구비를 살리려 하면 얼굴 전체를
  어둡게 깔아야 해서 실루엣까지 죽는다. 배치 CDF에 **곡률 크기**(부호 무시)를 곱해
  굴곡진 자리에 점을 몰아주면 같은 개수로 이목구비의 해상도만 올라간다. 상한을
  둘 것(`1 + min(1.7, k*5.5)`) — 없으면 눈·콧구멍 경계에만 뭉치고 이마·볼이 빈다.
  셰이더와 **같은 실루엣 가드**를 CPU 쪽에도 넣어야 한다(안 넣으면 턱선에 몰린다).
- **알파 하한이 대비를 잡아먹는다**: `alpha = 0.84 + lum*0.16`처럼 하한을 높게 두면
  그늘진 곳도 거의 최대 불투명도로 점을 찍어, 색을 아무리 어둡게 잡아도 additive로
  쌓이면 밝은 곳과 같은 톤이 된다. 하한은 밝기가 albedo 추정이 아니라 조명·곡률에서
  나오는 매끈한 값일 때 낮춰도 안전하다(0.34 + lum*0.66).
- **귀 마스크**: 타원 마스크의 `maskrx`는 리프레이밍 후 NDC 기준인데, 얼굴은 보통
  가로보다 세로가 길어 `half`가 세로로 잡힌다 — 즉 얼굴의 가로 NDC 범위는 1보다
  한참 작다. 0.63으로는 귀가 남는다(0.52 필요). 대비가 낮을 땐 안 보이다가 대비를
  올리는 순간 귀가 얼굴보다 먼저 눈에 들어온다.
