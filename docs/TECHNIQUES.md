...
- **`gl_PointSize`의 기준은 dpr이 아니라 프레임버퍼 높이**: `gl_PointSize`는 프레임버퍼
  픽셀 단위다. `size * devicePixelRatio / -mvPosition.z`로 쓰면 브라우저 확대 시
  dpr만 오르고 물리 창(=프레임버퍼)은 그대로라, 피사체 크기는 그대로인데 점만 커진다.
  additive 블렌딩에서는 점 면적이 곧 밝기라 **배율에 따라 밝기가 몇 배씩 흔들린다**
  (50%↔200%에서 5.1배 측정). three가 자기 `PointsMaterial`에서 쓰는 기준을 따를 것 —
  `WebGLMaterials.js`의 `uniforms.scale.value = height * 0.5`, 여기서 height는
  `canvas.height`(프레임버퍼 px)다. 확대하면 CSS 높이와 dpr이 반비례로 움직여 곱이
  불변이므로 점 크기가 고정된다. 카메라가 뒤로 물러나는 세로 화면에서도
  `-mvPosition.z`가 같이 커져 피사체 대비 비율이 유지된다.
- **점이 흐려 보이는 건 대개 겹침(overdraw)이다**: 초점이나 블러 문제가 아니다.
  점 지름이 **점 사이 간격보다 크면** 원리적으로 개별 점으로 분리될 수 없다
  (측정: 지름 5.1px / 간격 1.6px → 3배, 겹침 7.9배 = 화면 픽셀마다 반투명 원반
  8장). 진단은 `N × π(d/2)² ÷ 피사체 덮는 면적`, 목표는 겹침 1.5~2.5.
  **점 크기와 잉크 게인은 한 쌍으로 움직인다** — 크기를 줄이면 같은 면적을 덮는
  점 수가 제곱으로 줄어드니 게인을 그만큼 올려야 밝기가 유지된다(13/0.27 →
  7/0.85, 밝기 동일). 한쪽만 건드리면 어두워지거나 타버린다.
  겹침을 1 아래로 내리면 점이 아니라 **노이즈**가 된다 — 실루엣이 부서지고
  프레임간 반짝임이 커진다(120ms 프레임차 18.4 → 20.2 → 23.1/255로 증가).
  **게인을 올리면 게인에 안 묶인 다른 색 항이 상대적으로 묻힌다.** 파문 색처럼
  `mix()`의 목표색을 상수로 둔 항은 바탕만 밝아지고 자기는 그대로라 세기가
  1/(새 게인÷튜닝 당시 게인)로 줄어든다(1.3/0.27 = 4.8배 약화, G/R 0.87 → 0.46).
  게인에 비례시켜 상대 세기를 고정할 것.
- **작은 점은 경계 폭을 크기에 반비례시킬 것**: `smoothstep(0.5-B, 0.5+B, d)`의
  B를 상수로 두면 점이 작아질수록 절대 경계 폭이 같이 줄어 서브픽셀이 되고,
  파티클이 흐르는 동안 픽셀 격자에 걸렸다 빠지며 반짝인다. `B = 0.5/gl_PointSize`로
  화면상 약 1px을 유지한다(gl_PointSize가 프레임버퍼 단위라 레티나는 자동으로 더 날카롭다).
- **파티클 수 감축 분기를 CSS 픽셀로 재지 말 것**: `innerWidth < 760` 같은 기준은
  브라우저 확대가 잘못 밟는다 — CSS 픽셀은 확대하면 줄어들어 데스크톱 1280px 창도
  200%에서 640으로 읽힌다. 파티클이 뚝 떨어져 위 수정을 하고도 밝기가 튄다.
  `matchMedia('(pointer: coarse)')`는 배율과 무관해 기기 종류만 본다.
- **색 하나에서 팔레트 파생(색상 선택기)**: 바탕색만 바꾸면 하이라이트·파문색이
  그대로 남아 색이 서로 싸운다. 기존 팔레트에서 **색상환 간격과 채도·명도를 재서**
  그 관계를 그대로 옮기면(보라 #8302af 기준 하이라이트 −126°/S .77/L .52,
  파문 −124°/S .83/L .65) 기본값에서는 원래 색이 그대로 나오고 어떤 색을 골라도
  관계가 유지된다. 주의 둘: ① HSL 연산은 **sRGB에서** 할 것 —
  `getHSL/setHSL`의 기본은 작업 색공간(선형)이라 눈으로 본 색상환과 어긋난다.
  ② additive에서는 밝은 색을 고르면 타므로 **최대 채널을 안전 구간으로 클램프**
  (선형 0.24~0.58). 색상·채도는 고른 대로 두고 세기만 끌어온다.
- **전면 클릭 레이어 위의 UI**: 화면 전체를 덮는 클릭 핸들러(`@click="advance"`)가
  있는 페이지에서 컨트롤을 그 **자식으로 두면 클릭이 버블링**되어 조작할 때마다
  진행된다. 형제로 두고 z-index만 올릴 것(`.stop` 수식어보다 구조가 확실하다).
